...
def render_readme(inventory: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "",
        make_table(["Module", "Purpose"], sdk_surface_rows),
        "",
        (
            f"The current repository snapshot includes {counts['harnesses']} concrete harness manifests, "
            f"{counts['service_provider_packages']} service provider packages, "
            f"{counts['tool_only_external_surfaces']} tool-only external service surface"
            f"{'' if counts['tool_only_external_surfaces'] == 1 else 's'}, "
            f"and {counts['sink_types']} built-in output sink types."
        ),
        "",
