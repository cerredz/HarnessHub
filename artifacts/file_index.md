...
| `harnessiq/cli/` | adapters, commands, exa_outreach, instagram, leads, ledger, linkedin, master_prompts, prospecting, research_sweep | Argparse entrypoints and command-family modules for harness management plus ledger/output-sink operations. |
...
`tests/` currently contains 84 test modules. The table below groups them by dominant responsibility.

| Area | Count | Examples |
| --- | --- | --- |
| agents | 16 | `tests/test_agent_instances.py`, `tests/test_agents_base.py`, `tests/test_apollo_agent.py` |
