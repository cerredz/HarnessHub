...
import unittest
from harnessiq.providers.attio import AttioClient, AttioCredentials, build_attio_operation_catalog, get_attio_operation
...
class AttioClientTests(unittest.TestCase):
    ...
    def test_prepare_request_list_objects_url(self) -> None:
        prepared = self._client().prepare_request(ProviderOperationRequestDTO(operation='list_objects'))
    ...
    def test_prepare_request_interpolates_record_id(self) -> None:
        prepared = self._client().prepare_request(ProviderOperationRequestDTO(operation='get_record', path_params={'object': 'people', 'record_id': 'rec_1'}))
    ...
    def test_prepare_request_raises_on_missing_path_param(self) -> None:
        with self.assertRaises(ValueError):
            self._client().prepare_request(ProviderOperationRequestDTO(operation='get_record', path_params={'object': 'people'}))

    def test_prepare_request_raises_on_missing_required_payload(self) -> None:
        with self.assertRaises(ValueError):
            self._client().prepare_request(ProviderOperationRequestDTO(operation='create_record', path_params={'object': 'people'}))

    def test_prepare_request_rejects_payload_on_no_payload_op(self) -> None:
        with self.assertRaises(ValueError):
            self._client().prepare_request(ProviderOperationRequestDTO(operation='list_objects', payload={'bad': 'field'}))
...
class AttioToolsTests(unittest.TestCase):
    ...
    def test_allowed_operations_subset(self) -> None:
        creds = AttioCredentials(api_key='testkey')
        tools = create_attio_tools(credentials=creds, allowed_operations=['list_objects', 'get_record'])
