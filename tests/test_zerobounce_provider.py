...
from harnessiq.providers.zerobounce.operations import build_zerobounce_operation_catalog, build_zerobounce_request_tool_definition, create_zerobounce_tools, get_zerobounce_operation
...
class ZeroBounceOperationCatalogTests(unittest.TestCase):
    ...
    def test_bulk_operations_use_bulk_base(self):
        bulk_ops = {'bulk_send_file', 'bulk_file_status', 'bulk_get_file', 'bulk_delete_file', 'bulk_scoring_send_file', 'bulk_scoring_file_status', 'bulk_scoring_get_file', 'bulk_scoring_delete_file', 'bulk_finder_send_file', 'bulk_finder_file_status', 'bulk_finder_get_file', 'bulk_finder_delete_file'}
...
class ZeroBounceClientTests(unittest.TestCase):
    ...
    def _make_client(self, response=None):
        ...
        return ZeroBounceClient(credentials=creds, request_executor=_mock_executor(response or {'status': 'ok'}))

    def test_api_key_in_url_as_query_param(self):
        client = self._make_client()
        prepared = client.prepare_request(ProviderOperationRequestDTO(operation='validate_email', query={'email': 'test@test.com'}))
    ...
    def test_bulk_op_uses_bulk_base_url(self):
        client = self._make_client()
        prepared = client.prepare_request(ProviderOperationRequestDTO(operation='bulk_send_file', payload={'email_address_column': 1}))
        self.assertIn('bulkapi.zerobounce.net', prepared.url)

    def test_validate_batch_requires_payload(self):
        client = self._make_client()
        with self.assertRaises(ValueError) as ctx:
            client.prepare_request(ProviderOperationRequestDTO(operation='validate_batch'))
        self.assertIn('requires a payload', str(ctx.exception))

    def test_validate_email_does_not_accept_payload(self):
        client = self._make_client()
        with self.assertRaises(ValueError):
            client.prepare_request(ProviderOperationRequestDTO(operation='validate_email', payload={'bad': 'data'}))
    ...
    def test_execute_operation_returns_response(self):
        ...
        result = client.execute_operation(ProviderOperationRequestDTO(operation='validate_email', query={'email': 'test@test.com'})).response
...
class ZeroBounceToolTests(unittest.TestCase):
    ...
    def test_handler_returns_expected_shape(self):
        ...
        client = ZeroBounceClient(credentials=creds, request_executor=_mock_executor(response))
