...
class ToolRegistryTests(unittest.TestCase):
    def test_builtin_registry_keeps_stable_key_order(self) -> None:
        ...
        self.assertIn(TEXT_REGEX_EXTRACT, keys)
        self.assertIn(CONTROL_PAUSE_FOR_HUMAN, keys)
        self.assertGreater(keys.index(TEXT_REGEX_EXTRACT), keys.index(REASONING_ABDUCTIVE_REASONING))
        self.assertGreater(keys.index(CONTROL_PAUSE_FOR_HUMAN), keys.index(TEXT_REGEX_EXTRACT))
        self.assertGreater(len(keys), len(expected_prefix))
