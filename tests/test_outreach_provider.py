...
import unittest
from harnessiq.providers.outreach import OutreachClient, OutreachCredentials, build_outreach_operation_catalog, get_outreach_operation
...
class OutreachClientTests(unittest.TestCase):
    ...
    def test_prepare_request_list_prospects_url(self) -> None:
        prepared = self._client().prepare_request(ProviderOperationRequestDTO(operation='list_prospects'))
    ...
    def test_prepare_request_interpolates_prospect_id(self) -> None:
        prepared = self._client().prepare_request(ProviderOperationRequestDTO(operation='get_prospect', path_params={'prospect_id': 'p42'}))
    ...
    def test_prepare_request_raises_on_missing_path_param(self) -> None:
        with self.assertRaises(ValueError):
            self._client().prepare_request(ProviderOperationRequestDTO(operation='get_prospect'))

    def test_prepare_request_raises_on_missing_required_payload(self) -> None:
        with self.assertRaises(ValueError):
            self._client().prepare_request(ProviderOperationRequestDTO(operation='create_prospect'))

    def test_prepare_request_rejects_payload_on_no_payload_op(self) -> None:
        with self.assertRaises(ValueError):
            self._client().prepare_request(ProviderOperationRequestDTO(operation='list_prospects', payload={'bad': 'field'}))
