...
import unittest
from harnessiq.providers.exa import ExaClient, ExaCredentials, build_exa_operation_catalog, get_exa_operation
...
class ExaClientTests(unittest.TestCase):
    ...
    def test_prepare_request_search_url(self) -> None:
        prepared = self._client().prepare_request(ProviderOperationRequestDTO(operation='search', payload={'query': 'AI trends'}))
    ...
    def test_prepare_request_interpolates_webset_id(self) -> None:
        prepared = self._client().prepare_request(ProviderOperationRequestDTO(operation='get_webset', path_params={'webset_id': 'ws42'}))
        self.assertIn('ws42', prepared.url)

    def test_prepare_request_sets_x_api_key_header(self) -> None:
        prepared = self._client().prepare_request(ProviderOperationRequestDTO(operation='search', payload={'query': 'test'}))
        self.assertEqual(prepared.headers['x-api-key'], 'testkey')

    def test_prepare_request_raises_on_missing_path_param(self) -> None:
        with self.assertRaises(ValueError):
            self._client().prepare_request(ProviderOperationRequestDTO(operation='get_webset'))

    def test_prepare_request_raises_on_missing_required_payload(self) -> None:
        with self.assertRaises(ValueError):
            self._client().prepare_request(ProviderOperationRequestDTO(operation='search'))

    def test_prepare_request_rejects_payload_on_list_op(self) -> None:
        with self.assertRaises(ValueError):
            self._client().prepare_request(ProviderOperationRequestDTO(operation='list_websets', payload={'bad': 'field'}))
...
class ExaToolsTests(unittest.TestCase):
    ...
    def test_create_exa_tools_accepts_protocol_compatible_client(self) -> None:
        ...
        class _FakeClient:
            ...
            def prepare_request(self, request: ProviderOperationRequestDTO) -> _FakePreparedRequest:
