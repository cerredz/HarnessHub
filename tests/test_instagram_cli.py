...
class InstagramCliTests(unittest.TestCase):
    ...
    def test_run_invokes_agent_from_memory(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            mock_agent.instance_id = "instagram_keyword_discovery::abc12345"
            mock_agent.instance_name = "a"
            ...
            self.assertEqual(result, 0)
            payload = json.loads("".join(call.args[0] for call in mock_write.call_args_list))
            self.assertEqual(payload["instance_id"], "instagram_keyword_discovery::abc12345")
            self.assertEqual(payload["instance_name"], "a")
