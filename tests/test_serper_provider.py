...
import unittest
from harnessiq.providers.serper import SerperClient, SerperCredentials, build_serper_operation_catalog, get_serper_operation
...
class SerperClientTests(unittest.TestCase):
    ...
    def test_prepare_request_search_url(self) -> None:
        prepared = self._client().prepare_request(ProviderOperationRequestDTO(operation='search', payload={'q': 'AI trends'}))
    ...
    def test_prepare_request_sets_api_key_header(self) -> None:
        prepared = self._client().prepare_request(ProviderOperationRequestDTO(operation='news', payload={'q': 'AI'}))
    ...
    def test_prepare_request_rejects_path_params(self) -> None:
        with self.assertRaises(ValueError):
            self._client().prepare_request(ProviderOperationRequestDTO(operation='search', path_params={'bad': 'field'}, payload={'q': 'AI'}))
