...
from harnessiq.providers.lusha.operations import build_lusha_operation_catalog, build_lusha_request_tool_definition, create_lusha_tools, get_lusha_operation
...
class LushaClientTests(unittest.TestCase):
    ...
    def _make_client(self, response=None):
        ...
        return LushaClient(credentials=creds, request_executor=_mock_executor(response or {}))

    def test_api_key_in_headers(self):
        client = self._make_client()
        prepared = client.prepare_request(ProviderOperationRequestDTO(operation='enrich_person', query={'firstName': 'John', 'companyDomain': 'acme.com'}))
    ...
    def test_base_url_is_lusha(self):
        client = self._make_client()
        prepared = client.prepare_request(ProviderOperationRequestDTO(operation='get_account_usage'))
        self.assertIn('api.lusha.com', prepared.url)

    def test_missing_object_type_raises(self):
        client = self._make_client()
        with self.assertRaises(ValueError) as ctx:
            client.prepare_request(ProviderOperationRequestDTO(operation='get_signal_filters'))
        self.assertIn('object_type', str(ctx.exception))

    def test_object_type_path_substituted(self):
        client = self._make_client()
        prepared = client.prepare_request(ProviderOperationRequestDTO(operation='get_signal_filters', path_params={'object_type': 'contact'}))
    ...
    def test_bulk_enrich_persons_requires_payload(self):
        client = self._make_client()
        with self.assertRaises(ValueError):
            client.prepare_request(ProviderOperationRequestDTO(operation='bulk_enrich_persons'))

    def test_search_contacts_requires_payload(self):
        client = self._make_client()
        with self.assertRaises(ValueError):
            client.prepare_request(ProviderOperationRequestDTO(operation='search_contacts'))

    def test_get_account_usage_no_payload_accepted(self):
        client = self._make_client()
        with self.assertRaises(ValueError):
            client.prepare_request(ProviderOperationRequestDTO(operation='get_account_usage', payload={'bad': 'data'}))

    def test_execute_returns_response(self):
        response = {'total': 100, 'used': 25, 'remaining': 75}
        client = self._make_client(response=response)
        result = client.execute_operation(ProviderOperationRequestDTO(operation='get_account_usage')).response
...
class LushaToolTests(unittest.TestCase):
    ...
    def test_handler_returns_expected_shape(self):
        ...
        client = LushaClient(credentials=creds, request_executor=_mock_executor(response))
    ...
    def test_allowed_operations_filters_catalog(self):
        tools = create_lusha_tools(credentials=self._make_creds(), allowed_operations=['enrich_person', 'enrich_company'])
