...
from harnessiq.providers.apollo.operations import APOLLO_REQUEST, build_apollo_operation_catalog, get_apollo_operation
...
class ApolloClientTests(unittest.TestCase):
    ...
    def _client(self) -> ApolloClient:
        ...
        return ApolloClient(credentials=credentials, request_executor=lambda method, url, **kwargs: {'ok': True, 'url': url, 'method': method})

    def test_prepare_request_search_people_url(self) -> None:
        prepared = self._client().prepare_request(ProviderOperationRequestDTO(operation='search_people', payload={'person_titles': ['VP Sales']}))
    ...
    def test_prepare_request_interpolates_contact_id(self) -> None:
        prepared = self._client().prepare_request(ProviderOperationRequestDTO(operation='view_contact', path_params={'contact_id': 'contact-1'}))
    ...
    def test_prepare_request_includes_query_params(self) -> None:
        prepared = self._client().prepare_request(ProviderOperationRequestDTO(operation='enrich_organization', query={'domain': 'example.com'}))
        self.assertIn('domain=example.com', prepared.url)

    def test_prepare_request_rejects_missing_path_param(self) -> None:
        with self.assertRaises(ValueError):
            self._client().prepare_request(ProviderOperationRequestDTO(operation='view_contact'))

    def test_prepare_request_rejects_missing_payload(self) -> None:
        with self.assertRaises(ValueError):
            self._client().prepare_request(ProviderOperationRequestDTO(operation='search_people'))

    def test_prepare_request_rejects_payload_for_get(self) -> None:
        with self.assertRaises(ValueError):
            self._client().prepare_request(ProviderOperationRequestDTO(operation='view_contact', path_params={'contact_id': 'contact-1'}, payload={'bad': True}))
...
class ApolloToolsTests(unittest.TestCase):
    ...
    def test_tool_handler_executes_search_people(self) -> None:
        ...
        def fake(method: str, url: str, **kwargs: object) -> dict[str, object]:
            ...
            return {'people': []}
        client = ApolloClient(credentials=ApolloCredentials(api_key='testkey'), request_executor=fake)
        registry = ToolRegistry(create_apollo_tools(client=client))
        result = registry.execute(APOLLO_REQUEST, {'operation': 'search_people', 'payload': {'person_titles': ['VP Sales']}})
    ...
    def test_allowed_operations_subset(self) -> None:
        credentials = ApolloCredentials(api_key='testkey')
        tools = create_apollo_tools(credentials=credentials, allowed_operations=['search_people', 'view_usage_stats'])
