...
import unittest
from harnessiq.providers.instantly import InstantlyClient, InstantlyCredentials, build_instantly_operation_catalog, get_instantly_operation
...
class InstantlyClientTests(unittest.TestCase):
    ...
    def test_prepare_request_list_campaigns_url(self) -> None:
        prepared = self._client().prepare_request(ProviderOperationRequestDTO(operation='list_campaigns'))
    ...
    def test_prepare_request_interpolates_campaign_id(self) -> None:
        prepared = self._client().prepare_request(ProviderOperationRequestDTO(operation='get_campaign', path_params={'campaign_id': 'camp1'}))
    ...
    def test_prepare_request_raises_on_missing_path_param(self) -> None:
        with self.assertRaises(ValueError):
            self._client().prepare_request(ProviderOperationRequestDTO(operation='get_campaign'))

    def test_prepare_request_raises_on_missing_required_payload(self) -> None:
        with self.assertRaises(ValueError):
            self._client().prepare_request(ProviderOperationRequestDTO(operation='create_campaign'))

    def test_prepare_request_rejects_payload_on_no_payload_op(self) -> None:
        with self.assertRaises(ValueError):
            self._client().prepare_request(ProviderOperationRequestDTO(operation='list_campaigns', payload={'bad': 'field'}))
