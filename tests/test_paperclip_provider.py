...
import unittest
from harnessiq.providers.paperclip import DEFAULT_BASE_URL, PaperclipClient, PaperclipCredentials, build_headers, build_paperclip_operation_catalog, get_paperclip_operation
...
class PaperclipClientTests(unittest.TestCase):
    ...
    def test_prepare_request_renders_issue_list_url_and_query(self) -> None:
        prepared = self._client().prepare_request(ProviderOperationRequestDTO(operation='list_issues', path_params={'company_id': 'company-1'}, query={'status': 'todo,in_progress', 'assigneeAgentId': 'agent-42'}))
    ...
    def test_prepare_request_sets_run_id_header_on_mutation(self) -> None:
        prepared = self._client().prepare_request(ProviderOperationRequestDTO(operation='update_issue', path_params={'issue_id': 'issue-1'}, payload={'status': 'done', 'comment': 'Finished.'}, run_id='run-123'))
    ...
    def test_prepare_request_ignores_run_id_for_read_operation(self) -> None:
        prepared = self._client().prepare_request(ProviderOperationRequestDTO(operation='get_issue', path_params={'issue_id': 'issue-1'}, run_id='run-123'))
        self.assertNotIn('X-Paperclip-Run-Id', prepared.headers)

    def test_prepare_request_raises_on_missing_path_param(self) -> None:
        with self.assertRaises(ValueError):
            self._client().prepare_request(ProviderOperationRequestDTO(operation='get_company'))

    def test_prepare_request_raises_on_query_for_non_query_operation(self) -> None:
        with self.assertRaises(ValueError):
            self._client().prepare_request(ProviderOperationRequestDTO(operation='get_issue', path_params={'issue_id': 'issue-1'}, query={'status': 'todo'}))

    def test_prepare_request_raises_on_missing_required_payload(self) -> None:
        with self.assertRaises(ValueError):
            self._client().prepare_request(ProviderOperationRequestDTO(operation='create_issue', path_params={'company_id': 'company-1'}))

    def test_prepare_request_rejects_payload_for_payloadless_operation(self) -> None:
        with self.assertRaises(ValueError):
            self._client().prepare_request(ProviderOperationRequestDTO(operation='list_companies', payload={'unexpected': True}))
    ...
    def test_execute_operation_delegates_to_request_executor(self) -> None:
        ...
        def fake_request(method: str, url: str, **kwargs: object) -> dict[str, object]:
            ...
            return {'id': 'company-1'}
        result = self._client(request_executor=fake_request).execute_operation(ProviderOperationRequestDTO(operation='list_companies')).response
...
class PaperclipToolsTests(unittest.TestCase):
    ...
    def test_tool_handler_executes_operation(self) -> None:
        ...
        result = registry.execute(PAPERCLIP_REQUEST, {'operation': 'update_issue', 'path_params': {'issue_id': 'issue-1'}, 'payload': {'status': 'done', 'comment': 'Finished.'}, 'run_id': 'run-123'})
