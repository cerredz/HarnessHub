...
Intent:
Close the gap between â€œa job can be deployedâ€ and â€œa harness can run repeatedly in Cloud Run with continuity.â€ This ticket introduces the remote runtime path that can execute any manifest-backed harness and synchronize its durable memory/profile state with GCS.
...
Approach:
Use the manifest-driven deploy spec from Ticket 14 to resolve the remote run. Before the harness starts, sync the relevant memory/profile state from GCS into the local working directory; after the run, sync updated state back to GCS. This adapts the design docâ€™s memory-persistence goal to the actual repository architecture, which persists many files per harness rather than one universal memory document.
...
Acceptance Criteria:
- [ ] A generic cloud runtime wrapper can resolve a stored GCP deploy config and invoke the correct harness run path.
- [ ] Harness profile and durable memory state can be synced from GCS before execution and back to GCS after execution.
- [ ] The runtime wrapper is generic across built-in manifests rather than hard-coded to one harness.
- [ ] Tests cover sync-in, sync-out, and failure-handling behavior without live GCP access.
