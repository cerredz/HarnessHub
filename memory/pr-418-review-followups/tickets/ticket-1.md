...
Acceptance Criteria:
- [ ] The shared formalization record/spec types live under `harnessiq/shared/`.
- [ ] The interface layer uses a `harnessiq/interfaces/formalization/` package instead of one monolithic module.
- [ ] Each formalization class/base concern lives in its own file with large top-of-file and class-level explanatory comments.
- [ ] Default description text is materially richer and explains what, why, how, and intent for the injected formalization layers.
- [ ] Generated docs are updated through `scripts/sync_repo_docs.py`, including file-index visibility for the formalization-layer package.
- [ ] Targeted interface, shared, and docs-sync tests pass on the final branch.
