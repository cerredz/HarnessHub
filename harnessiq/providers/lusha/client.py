"""Lusha credentials and HTTP client."""
...
from harnessiq.providers.lusha.api import DEFAULT_BASE_URL
from harnessiq.providers.http import RequestExecutor, request_json


@dataclass(frozen=True, slots=True)
class LushaCredentials:
    """Runtime credentials for the Lusha B2B contact intelligence API.

    ``api_key`` is passed via the ``api_key`` request header (lowercase,
    case-sensitive) on every request.  Obtain it from the Lusha dashboard
    at dashboard.lusha.com/enrich/api.
    """
...
@dataclass(frozen=True, slots=True)
class LushaClient:
    """Minimal Lusha HTTP client suitable for tool execution and tests."""
    ...
    def prepare_request(
        ...
        *,
        ...
        from harnessiq.providers.lusha.operations import _build_prepared_request
    ...
    def execute_operation(
        ...
        *,
        ...
        """Execute one validated Lusha operation and return the decoded response."""
